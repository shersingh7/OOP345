...
int main(int argc, char** argv)
{
	...
	if (argc == 5) {
		...
		std::string book;
		while (library.size() < 4 && std::getline(fileBook, book)) {
			if (book[0] != '#') {
				library += Book(book);
			}
		}
		...
		/*
		 ô Hey, I just met you,      ô
		 ô And this is crazy,        ô
		 ô But here's my number.     ô    (register the observer)
		 ô So, if something happens, ô    (event)
		 ô Call me, maybe?           ô    (callback)
		 */
		library.setObserver(bookAddedObserver);

		// TODO: add the rest of the books from the file.
		while (std::getline(fileBook, book)) {
			if (book[0] != '#') {
				library += Book(book);
			}
		}
	...
	if (argc > 2) {
		...
		//       - lines that start with "#" are considered comments and should be ignored
		std::ifstream fileMovie(argv[2]);

		if (!fileMovie) {
			std::cerr << "Cant open file : " << argv[2] << std::endl;
			exit(AppErrors::CannotOpenFile);
		}
		...
		for (size_t i = 0; i < 5; i++) {
			std::getline(fileMovie, movieLine);
			if (movieLine[0] != '#') {
				movies[i] = Movie(movieLine);
			}
			else {
				i--;
			}
