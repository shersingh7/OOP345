...
int main(int argc, char* argv[])
{
	...
	{
		sdds::SetSummable<15, sdds::PairSummable<string, string>> products;
		...
		// get the data from the file
		{
			...
			do
			{
				...
				if (input)
				{
					sdds::PairSummable<string, string> record(label, value);
					products += record;
				}
			} while (input);
			input.close();
	...
	{
		sdds::SetSummable<25, sdds::PairSummable<int, string>> ticketSales;
		...
		// get the data from the file
		{
			...
			do
			{
				...
				if (input)
				{
					sdds::PairSummable<int, string> record(number, ticketType);
					ticketSales += record;
				}
			} while (input);
			input.close();
